...
struct Params {
    ...
    // Plan B: 顏色調整強度（0.0 ~ 1.0）
    double color_adjust_strength = 0.0;
};
...
void readInitFile(const string& init_file, Params& params)
{
    try {
        ...
        // Plan B: color_adjust_strength (optional)
        if (strings.size() > 8) {
    ...
    if (p.mosaicize) {
        ...
        for (int i = 0; i < src.rows; ++i) {
            for (int j = 0; j < src.cols; ++j) {
                ...
                Vec3b color = src(i, j); // 目標 pixel 顏色
                ...
                // --- Plan B: 顏色插值調整 ---
                if (p.color_adjust_strength > 0.0) {
                    // 將 color 轉成 float scalar
                    Scalar targetColor(
                    ...
                    // p' = (1 - s) * p + s * targetColor
                    double s = std::min(std::max(p.color_adjust_strength, 0.0), 1.0);
                    Mat targetMat(pixel.size(), CV_32FC3, targetColor);

                    pixelFloat = (1.0 - s) * pixelFloat + s * targetMat;
                    ...
                    Mat pixelAdjusted;
                    pixelFloat.convertTo(pixelAdjusted, CV_8UC3);
                    pixel = pixelAdjusted;
                }
